...
<?php else: ?>
    ...
    function addApp(){ const data={name:$('#appName').val(),game:$('#appGame').val(),api_url:$('#appApiUrl').val(),api_type:$('#appApiType').val(),duration_hours:$('#appDuration').val(),maintenance:$('#appMaintenance').is(':checked')?'1':'0'}; if(!data.name||!data.game||!data.api_url){alert('All fields are required.');return;} api('add_application',data,refreshIfOk); }
