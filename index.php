...
<head>
    ...
    <style>
        ...
        .license-card .card-footer { padding:16px 26px; padding-bottom:calc(16px + env(safe-area-inset-bottom, 0px)); }
        .license-card #btn_submit:disabled { cursor:not-allowed; opacity:.58; box-shadow:none; }
        ...
        @media (max-width:767px) { .home-hero { padding:28px 0 22px; } .home-hero h1 { font-size:clamp(2.1rem,12vw,3.25rem); } .license-card .card-header, .license-card .card-body { padding:21px; } .license-card .card-footer { padding:15px 21px; padding-bottom:calc(15px + env(safe-area-inset-bottom, 0px)); } .side-panel { display:block; padding:20px; } .side-item { border-bottom:1px solid var(--hp-border); border-left:0; border-right:0; border-radius:0; padding:12px 0; } .application-trigger { min-height:64px; padding:10px 12px; } .trigger-content { gap:10px; } .trigger-title, .trigger-subtitle { max-width:calc(100vw - 150px); } .application-menu { max-height:min(360px,55vh); overflow-y:auto; } }
    </style>
...
<body>
    ...
    (function(){
        ...
        $('.application-option').on('click',function(){ var id=String($(this).data('app-id')), option=$('#app_id option[value="'+id+'"]'), logo=$(this).find('img').attr('src'), name=$(this).find('.menu-name').text(), game=$(this).find('.menu-meta').text(), duration=option.data('duration')||5; $('#app_id').val(id); $('#duration').prop('disabled',false).html('<option value="'+duration+'" selected>'+duration+' Hours</option>'); $('#btn_submit').prop('disabled',false).attr('aria-disabled','false'); $('.application-option').removeClass('selected').attr('aria-selected','false'); $(this).addClass('selected').attr('aria-selected','true'); $('#selectionHint').text(name); $('#triggerTitle').text(name); $('#triggerSubtitle').text(game); $('#triggerLogo').html(logo?'<img src="'+escapeHtml(logo)+'" alt="">':'<i class="bi bi-box-seam"></i>'); closeMenu(); });
        $('#licenseForm').on('submit',function(e){ e.preventDefault(); var selected=$('#app_id option:selected'), appId=selected.val(); if(!appId){ $('#btn_submit').prop('disabled',true).attr('aria-disabled','true'); $('#duration').prop('disabled',true).html('<option value="" selected>Select Duration</option>'); $('#validationResult').html('<div class="alert alert-warning mb-0"><i class="bi bi-exclamation-triangle me-1"></i>Please select an application.</div>'); return; } if(selected.data('maintenance')==1){ $('#validationResult').html('<div class="alert alert-warning mb-0">This API is under maintenance. Choose another API.</div>'); return; } $('#btn_submit').prop('disabled',true).html('<span class="spinner-border spinner-border-sm me-1"></span>Generating...'); $('#validationResult').html(''); $.ajax({url:'api.php?action=generate',method:'POST',data:{app_id:appId},dataType:'json',success:function(response){ if(response.success){ window.location.href='redirect.php?handoff='+encodeURIComponent(response.handoff); }else{ $('#btn_submit').prop('disabled',false).attr('aria-disabled','false').html('<i class="bi bi-key-fill me-1"></i>Generate'); $('#validationResult').html('<div class="alert alert-danger mb-0"><i class="bi bi-exclamation-triangle me-1"></i>'+escapeHtml(response.error||'Failed to generate license')+'</div>'); } },error:function(){ $('#btn_submit').prop('disabled',false).attr('aria-disabled','false').html('<i class="bi bi-key-fill me-1"></i>Generate'); $('#validationResult').html('<div class="alert alert-danger mb-0"><i class="bi bi-wifi-off me-1"></i>An error occurred. Please try again.</div>'); }}); });
