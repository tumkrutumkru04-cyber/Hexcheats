...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .table td, .table th { padding:.78rem .65rem; }
        .app-logo-cell { width:32px; height:32px; border-radius:9px; display:inline-grid; place-items:center; overflow:hidden; flex:0 0 32px; background:linear-gradient(135deg,#7c6cff,#31d6c6); color:#fff; }
        .app-logo-cell img { width:100%; height:100%; object-fit:cover; }
        .btn:focus-visible, .side-link:focus-visible, .form-control:focus-visible, .form-select:focus-visible { outline:3px solid rgba(22,119,255,.22); outline-offset:2px; }
        @media (max-width: 991px) { .sidebar { transform:translateX(-100%); transition:transform .2s ease; box-shadow:12px 0 28px rgba(16,24,40,.18); }.sidebar.open { transform:none; }.sidebar-backdrop { display:block; position:fixed; inset:0; background:rgba(16,24,40,.38); opacity:0; pointer-events:none; transition:opacity .2s ease; z-index:1020; }.sidebar-backdrop.show { opacity:1; pointer-events:auto; }.main { margin-left:0; width:100%; }.mobile-top { display:block; }.topbar { padding:0 18px; }.content { padding:22px 18px 38px; }.section-head { align-items:center; }.table-responsive { overflow-x:auto; -webkit-overflow-scrolling:touch; } }
        @media (max-width: 575px) { .topbar { height:68px; }.page-title h1 { font-size:1.08rem; }.page-title p { display:none; }.metric .value { font-size:1.65rem; }.section-card { padding:16px; }.content { padding:16px 12px 32px; }.security-banner { flex-direction:column; }.security-banner .btn { margin-left:0 !important; }.section-head { flex-direction:column; }.section-head .btn { width:100%; }.metric { min-height:112px; padding:15px; } }
    </style>
...
<?php else: ?>
    <div class="shell">
        ...
        <main class="main">
            ...
            <div class="content">
                ...
                <section id="applications" class="surface section-card mb-4"><div class="section-head"><div><h2>Application management</h2><p>Control remote key providers and public availability.</p></div><button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addAppModal"><i class="bi bi-plus-lg me-1"></i>Add application</button></div><div class="table-responsive"><table class="table"><thead><tr><th>ID</th><th>Application</th><th>Game</th><th>Provider</th><th>Duration</th><th>Maintenance</th><th>Status</th><th>Actions</th></tr></thead><tbody><?php if (!$appsData): ?><tr><td colspan="8"><div class="empty">No applications configured.</div></td></tr><?php else: foreach ($appsData as $app): ?><tr><td class="dark-muted">#<?php echo (int)$app['id']; ?></td><td><div class="d-flex align-items-center gap-2"><span class="app-logo-cell"><?php if (!empty($app['logo'])): ?><img src="<?php echo htmlspecialchars($app['logo'], ENT_QUOTES); ?>" alt=""><?php else: ?><i class="bi bi-box-seam"></i><?php endif; ?></span><span class="fw-semibold"><?php echo htmlspecialchars($app['name']); ?></span></div></td><td><?php echo htmlspecialchars($app['game']); ?></td><td><span class="badge-soft <?php echo ($app['api_type'] ?? 'default') === 'moco' ? 'amber' : (($app['api_type'] ?? 'default') === 'hexgen' ? 'green' : 'blue'); ?>"><?php echo htmlspecialchars(strtoupper($app['api_type'] ?? 'default')); ?></span></td><td><?php echo (int)($app['duration_hours'] ?? 5); ?> hours</td><td><span class="badge-soft <?php echo !empty($app['maintenance']) ? 'amber' : 'green'; ?>"><?php echo !empty($app['maintenance']) ? 'ON' : 'OFF'; ?></span></td><td><span class="badge-soft <?php echo ($app['status'] ?? '') === 'active' ? 'green' : 'red'; ?>"><?php echo htmlspecialchars(ucfirst($app['status'] ?? 'inactive')); ?></span></td><td><button class="btn btn-light btn-icon me-1" title="Edit" onclick="editApp(<?php echo (int)$app['id']; ?>)"><i class="bi bi-pencil"></i></button><button class="btn btn-outline-danger btn-icon" title="Delete" onclick="deleteApp(<?php echo (int)$app['id']; ?>)"><i class="bi bi-trash3"></i></button></td></tr><?php endforeach; endif; ?></tbody></table></div></section>
    ...
    <div class="modal fade" id="addAppModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Add application</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label">Application name</label><input id="appName" class="form-control"></div><div class="mb-3"><label class="form-label">Game name</label><input id="appGame" class="form-control"></div><div class="mb-3"><label class="form-label">Application logo URL <span class="dark-muted fw-normal">(optional)</span></label><input id="appLogo" class="form-control" placeholder="https://example.com/logo.png"></div><div class="mb-3"><label class="form-label">Provider API URL</label><input id="appApiUrl" class="form-control" placeholder="https://provider.example/api"></div><div class="row g-3"><div class="col-6"><label class="form-label">API type</label><select id="appApiType" class="form-select"><option value="default">Default</option><option value="moco">MOCO</option><option value="hexgen">HEXGEN</option></select></div><div class="col-6"><label class="form-label">Duration (hours)</label><input id="appDuration" class="form-control" type="number" min="1" max="8760" value="5"></div></div><div class="form-check form-switch mt-3"><input class="form-check-input" type="checkbox" id="appMaintenance"><label class="form-check-label" for="appMaintenance">Put this API in maintenance</label></div></div><div class="modal-footer"><button class="btn btn-light" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary" onclick="addApp()">Add application</button></div></div></div></div>
    ...
    function addApp(){ const data={name:$('#appName').val(),game:$('#appGame').val(),logo:$('#appLogo').val(),api_url:$('#appApiUrl').val(),api_type:$('#appApiType').val(),duration_hours:$('#appDuration').val(),maintenance:$('#appMaintenance').is(':checked')?'1':'0'}; if(!data.name||!data.game||!data.api_url){alert('All fields are required.');return;} api('add_application',data,refreshIfOk); }
    function editApp(id){ const app=APPS.find(a=>Number(a.id)===Number(id)); if(!app)return; const name=prompt('Application name:',app.name); if(name===null)return; const game=prompt('Game name:',app.game); if(game===null)return; const logo=prompt('Application logo URL (optional):',app.logo||''); if(logo===null)return; const api_url=prompt('Provider API URL:',app.api_url); if(api_url===null)return; const api_type=prompt('API type (default, moco, or hexgen):',app.api_type||'default'); if(api_type===null)return; const duration_hours=prompt('Duration in hours:',app.duration_hours||5); if(duration_hours===null)return; const maintenance=confirm('Press OK to enable maintenance for this API. Press Cancel to keep it online.')?'1':'0'; const status=confirm('Press OK to keep this application active. Press Cancel to disable it.')?'active':'inactive'; api('edit_application',{id,name,game,logo,api_url,api_type,duration_hours,maintenance,status},refreshIfOk); }
    ...
    document.querySelectorAll('.side-link[href^="#"]').forEach(function(link){link.addEventListener('click',function(){document.querySelectorAll('.side-link').forEach(x=>x.classList.remove('active'));link.classList.add('active');closeSidebar();});});
    ...
    document.getElementById('menuToggle')?.addEventListener('click',function(){document.getElementById('sidebar')?.classList.toggle('open');document.getElementById('sidebarBackdrop')?.classList.toggle('show');});
